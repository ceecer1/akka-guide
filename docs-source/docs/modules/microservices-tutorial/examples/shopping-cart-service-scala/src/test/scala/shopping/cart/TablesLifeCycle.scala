...
trait TablesLifeCycle extends BeforeAndAfterAll { this: Suite =>
  ...
  override protected def beforeAll(): Unit = {
    ...
    Await.result(
      for {
        _ <- r2dbcExecutor.executeDdl("beforeAll dropTable")(_.createStatement(tablesSql(dropTablesFile)))
        _ <- r2dbcExecutor.executeDdl("beforeAll createTable")(_.createStatement(tablesSql(createTablesFile)))
      } yield Done,
      30.seconds)
    if (createUserTablesFile.exists()) {
      Await.result(
        for {
          _ <- r2dbcExecutor.executeDdl("beforeAll dropTable")(_.createStatement("DROP TABLE IF EXISTS public.item_popularity;"))
          _ <- r2dbcExecutor.executeDdl("beforeAll dropTable")(_.createStatement(tablesSql(createUserTablesFile)))
        } yield Done,
        30.seconds)
    }
    LoggerFactory
